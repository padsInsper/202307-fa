...
- colocar base simulada no github
- leitura de dados no github
- revisar scripts em R
- montar scripts em python com sns e outras coisas de interesse (média móvel e descritivas)
- fazer o prophet funcionar no python
...
dates = pd.date_range("2005-06-01", periods = 8187, freq = "D")

serie_dados = pd.Series(dados['y'].array, index = dates)
serie_dados.plot()
```
...
set.seed(7)
dados <- tsibbledata::aus_retail |>
  dplyr::filter(
    `Series ID` == sample(`Series ID`, 1)
  ) |>
  dplyr::select(Month, Turnover)

x <- ts(dados, start = c(1982, 4), frequency = 12)
...
acf(diff(dados_ts_exemplos[,"vendas"]))
```
...
dados = pd.read_csv("dados_lab01.csv")
