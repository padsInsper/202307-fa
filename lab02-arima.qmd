...
dados_raw <- "Transferencias---Dados-Consolidados.xlsx" |>
  readxl::read_excel(1, "C38:OC38", col_names = FALSE,
                     .name_repair = "minimal") |>
  janitor::clean_names() |>
  tidyr::pivot_longer(dplyr::everything()) |>
  dplyr::mutate(
    date = seq(as.Date("1991-01-01"), as.Date("2023-07-01"), "1 month"),
    value = value / 1e9
  ) |>
  dplyr::select(-name) |>
  tidyr::fill(value) |>
...
fit <- tdata |>
  model(
    arima_manual = ARIMA(value ~ 1 + pdq(2,0,2) + PDQ(2,1,1)),
